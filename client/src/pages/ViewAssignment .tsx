...
const ViewAssignment: React.FC = () => {
    ...
    useEffect(() => {
        const fetchAssignment = async () => {
            try {
                const { data } = await axios.get(`http://localhost:8080/api/classrooms/assignments/${assignmentId}`, { withCredentials: true });
    ...
    return (
        <div className="max-w-3xl mx-auto p-6 bg-white shadow-md rounded-lg">
            ...
            {assignment.filePath && (
                <a href={assignment.filePath} target="_blank" rel="noopener noreferrer" className="text-blue-600">
                    Download Assignment
                </a>
            )}
            <p className="mt-2"><strong>Due Date:</strong> {assignment.dueDate}</p>

            {/* Submission Upload */}
            <div className="mt-6">
                <label className="block text-gray-700 font-medium">Upload Submission</label>
                <div
                ...
                <button
                    onClick={handleSubmit}
                    className={`mt-4 px-4 py-2 rounded-lg ${isLate ? "bg-red-600" : "bg-blue-600"} text-white`}
                >
